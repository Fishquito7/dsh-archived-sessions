...
function parseArgs(argv: string[]): { command: string | undefined; sessionId?: string; options: CliOptions } {
  let offline = false;
  let json = false;
  let yes = false;
  let port = 0;
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (arg === "--offline") offline = true;
      ...
      port = Number(argv[++i]);
      if (!Number.isInteger(port) || port <= 0 || port > 65535) fail(`无效端口：${argv[i]}`);
    } else if (arg.startsWith("--port=")) {
      port = Number(arg.slice("--port=".length));
      if (!Number.isInteger(port) || port <= 0 || port > 65535) fail(`无效端口：${arg}`);
    } else if (arg === "--profile") {
      profile = argv[++i];
      if (profile === undefined || profile === "") fail("--profile 需要一个配置名参数");
    } else if (arg.startsWith("--profile=")) {
      profile = arg.slice("--profile=".length);
      if (profile === "") fail("--profile 需要一个配置名参数");
...
async function discoverPort(explicit: number): Promise<number> {
...
async function findSessionDir(id: string): Promise<string | undefined> {
  ...
  for (const project of projects) {
    ...
    for (const entry of entries) {
      if (entry.isDirectory() && entry.name === encoded) return join(root, project.name, entry.name);
    }
  }
  return undefined;
}

async function loadWorkspaceDoc(): Promise<WorkspaceDoc> {
  const file = dshHomePath("storages", "workspace.json");
  try {
    return JSON.parse(await readFile(file, "utf8")) as WorkspaceDoc;
  } catch (error: any) {
    if (error?.code === "ENOENT") fail("未找到 workspace 存储文件；DSH 可能尚未初始化。");
    throw error;
  }
}
 ...
 */
async function mutateWorkspaceDoc(mutator: (doc: WorkspaceDoc) => void): Promise<void> {
  const file = dshHomePath("storages", "workspace.json");
  await withFileLock(file, async () => {
    const current = JSON.parse(await readFile(file, "utf8")) as WorkspaceDoc;
    mutator(current);
    await writeFileAtomic(file, JSON.stringify(current, null, 2) + "\n", { mode: 0o600 });
  });
}

function archivedIdsOf(doc: WorkspaceDoc): string[] {
  const ids = doc.global?.archivedSessionIds;
  return Array.isArray(ids) ? ids.map(String) : [];
}

function workspaceForSession(doc: WorkspaceDoc, sessionId: string): WorkspaceRecord | undefined {
  for (const record of Object.values(doc.tables?.workspaces ?? {})) {
    if ((record.sessionIds ?? []).some((id) => String(id) === sessionId)) return record;
  }
  return undefined;
}
...
type OnlineAttempt =
  | { ok: true; value: any }
  | { ok: false; reason: "down" | "running" | "error"; message: string };

async function onlineCall(port: number, method: string, sessionId?: string): Promise<any> {
  ...
  if (body?.type !== "server-response") throw new Error("DSH web 返回了无法识别的响应信封");
  if (!body.result?.ok) throw new Error(body.result?.error?.message ?? `调用 ${endpoint} 失败`);
  return body.result.value;
}

async function tryOnline(port: number, method: string, sessionId?: string): Promise<OnlineAttempt> {
  try {
...
function printRows(rows: ArchivedCliRow[], json: boolean, offline: boolean): void {
  if (json) {
  ...
  for (const [index, row] of rows.entries()) {
    ...
    console.log(`    日志:   ${file}`);
    if (row.live) console.log(`    状态:   运行中`);
  }
}
...
async function offlineRows(): Promise<ArchivedCliRow[]> {
  const doc = await loadWorkspaceDoc();
  const rows: ArchivedCliRow[] = [];
...
function runPluginAdd(profile: string, downloadUrl: string): Promise<number> {
...
async function commandUpdate(options: CliOptions): Promise<void> {
  ...
  if (exit === 0) console.log(`更新完成：v${release.version}（宿主端有改动时需重启网关生效）`);
  process.exitCode = exit;
}

async function commandList(options: CliOptions, port: number): Promise<void> {
  if (options.offline) {
  ...
  if (online.ok) {
    ...
    printRows(sessions.map((row: any) => ({
      id: String(row.id),
...
async function commandRestore(sessionId: string | undefined, options: CliOptions, port: number): Promise<void> {
  if (sessionId === undefined || sessionId.trim() === "") fail("restore 需要一个会话 id");
  if (!options.offline) {
    ...
    if (online.reason !== "down") fail(online.message);
  }

  // 离线恢复：只从 archivedSessionIds 移除，工作区席位保留。
  await mutateWorkspaceDoc((doc) => {
    const archived = archivedIdsOf(doc);
    if (!archived.includes(sessionId)) throw new Error(`会话不在已归档列表中：${sessionId}`);
...
async function commandDelete(sessionId: string | undefined, options: CliOptions, port: number): Promise<void> {
  if (sessionId === undefined || sessionId.trim() === "") fail("delete 需要一个会话 id");

  const onlineForInfo = options.offline ? { ok: false as const, reason: "down" as const, message: "" } : await tryOnline(port, "list");
  let workspaceLabel: string | undefined;
  if (onlineForInfo.ok) {
    const row = (onlineForInfo.value?.sessions ?? []).find((item: any) => String(item.id) === sessionId);
  ...
  if (!options.yes) {
    if (!process.stdin.isTTY) fail("非交互终端删除需要显式传入 --yes");
...
async function main(): Promise<void> {
  ...
  if (command === "list") await commandList(options, port);
  else if (command === "restore") await commandRestore(sessionId, options, port);
  else if (command === "delete") await commandDelete(sessionId, options, port);
  else if (command === "update") await commandUpdate(options);
