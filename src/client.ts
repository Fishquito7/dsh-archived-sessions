(window as any).__ModuleLoader__.load({
  ...
  factory: (require: any) => {
    ...
    const codec = (symbol: string) => ({ mode: "strict", typeSymbol: symbol, schema: { parse: identity } });
    ...
    // ── 设置页面组件 ─────────────────────────────────────────────────────────
    function ArchivedSessionsSection(props: any) {
      ...
      react.useEffect(() => {
        ...
        void Promise.resolve()
          ...
          .then((info: any) => {
            if (active && info?.updateAvailable === true) setUpdateInfo(info);
          }, () => {});
        return () => {
          active = false;
        };
      ...
      const run = async (id: string, action: () => Promise<void>, doneText: string) => {
            ...
            : state?.status === "error"
                    ...
                    (0, react_jsx_runtime.jsx)("button", {
                      ...
                      onClick: () => {
                        void (typeof refreshWorkspaces === "function" ? refreshWorkspaces() : Promise.resolve()).then(() => {
                          if (typeof refreshSessions === "function") void refreshSessions();
                        });
                      },
                      children: t("retry")
                    })
              ...
              : rows.length === 0
                ...
                : (0, react_jsx_runtime.jsx)("ul", {
                    ...
                    children: rows.map((row: ArchivedRow) => {
    ...
    const patchArchivedNavIcon = () => {
      ...
      for (const dialog of document.querySelectorAll('[role="dialog"]')) {
        for (const button of dialog.querySelectorAll("button")) {
          if (button.dataset.archivedNav === "1") continue;
          let hit = false;
          ...
          if (hit) button.dataset.archivedNav = "1";
        }
      }
    };
    ...
    function apply(ctx: any) {
      ...
      const sectionFace = () => {
        ...
        let rowsCache: ArchivedRow[] = [];
        let stateCache: { status: "loading" | "ready" | "error" } = { status: "loading" };
        ...
        const build = () => {
          ...
          if (workspaceFailed || sessionFailed) stateCache = { status: "error" };
          else if (workspaceState?.state === "loading" || sessionState?.state === "loading") stateCache = { status: "loading" };
          else stateCache = { status: "ready" };
        };
        ...
        const face = {
          ...
          refreshSessions: () => {
            if (typeof sessions.refresh === "function") return sessions.refresh();
            return Promise.resolve();
          },
          refreshWorkspaces: () => {
            if (typeof workspaces.refresh === "function") return workspaces.refresh();
            return Promise.resolve();
          },
          restore: (sessionId: string) => callRemote("restore", sessionId),
