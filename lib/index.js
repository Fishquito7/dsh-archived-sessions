...
function assertPatchable(registry) {
 ...
 */
function installUnarchivePatch(ctx, registry) {
    ...
    const implementation = async function unarchiveSession(sessionId) {
    ...
    return () => {
        if (registry.unarchiveSession === implementation) {
...
class ArchivedSessionsGateway extends TypertRemoteService {
    ...
    async unarchive(sessionId) {
        const fn = this.registry.unarchiveSession;
    ...
    async list() {
        ...
        for (const header of headers)
            headerById.set(String(header.id), header);
        ...
        const liveIds = new Set(this.sessions.list().map((session) => String(session.id)));
...
export function apply(ctx) {
    ...
    ctx.effect(() => ctx.skills.register({ ...embeddedSkill, source: "runtime" }), "archived-sessions: embedded CLI skill");
}
